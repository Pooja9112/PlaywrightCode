...
test.beforeAll(async ({browser})=>
{
    ...
    await context.storageState({path:'state.json'}); //Save storage state to file
    webContext = await browser.newContext({storageState: 'state.json'}); //Create a new context with the saved storage state
});
test("Assignment login page", async() =>
{
    const productname = "ZARA COAT 3";
    const email = 'pooja1@example.com';
    const page= await webContext.newPage();//Ensure that WebContext is already initialized
    await page.goto("https://rahulshettyacademy.com/client");
